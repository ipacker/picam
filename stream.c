...
static const int is_clock_enabled = 0;
...
static int is_hdmi_enabled;
static const int is_hdmi_enabled_default = 1;
...
static int setup_hdmi_input() {
  ...
  hdmi_frame_width = ((i2c_rd8(i2c_fd, H_SIZE_HI) & 0x1f) << 8) +
  ...
  hdmi_output->format->es->video.width = VCOS_ALIGN_UP(hdmi_width, 32); //VCOS_ALIGN_UP(WIDTH, 32);
  hdmi_output->format->es->video.height = VCOS_ALIGN_UP(hdmi_height, 16); //VCOS_ALIGN_UP(HEIGHT, 16);
  hdmi_output->format->es->video.frame_rate.num = 10000;
  hdmi_output->format->es->video.frame_rate.den = hdmi_frame_interval ? hdmi_frame_interval : 10000;
  hdmi_output->format->encoding = ENCODING;
  hdmi_status = mmal_port_format_commit(hdmi_output);
  ...
  //running = 1;
  int i;
  for(i=0; i<hdmi_splitter_output2->buffer_num; i++)
  {
     MMAL_BUFFER_HEADER_T *buffer = mmal_queue_get(hdmi_pool->queue);
...
static int video_encode_startup() {
  ...
  if(is_hdmi_enabled) {
    ...
    portdef.format.video.xFramerate = hdmi_fps * 65536;
    portdef.format.video.nBitrate = 0x0;
    portdef.format.video.nSliceHeight = (hdmi_height+15)&~15;
    portdef.format.video.nStride = (hdmi_width+31)&~31;
  ...
  print_def(portdef_encode_output);
...
static void encode_and_send_image(MMAL_BUFFER_HEADER_T *mbuf) {
  ...
  if(mbuf) {
    mmalil_buffer_header_to_omx(buf, mbuf);
  } else {
    #if ENABLE_PBUFFER_OPTIMIZATION_HACK
      if (video_encode_input_buf == NULL) {
...
int main(int argc, char **argv) {
  ...
  is_hdmi_enabled = is_audio_preview_enabled_default;
